...
try {
    ...
    for(`$iteration = 0; `$iteration -lt $iterationMax; `$iteration++) { 
   
      if (-not `$IsWindows) { 
        `$start = [datetime]::now 
        `$null = Start-Process -FilePath "$pwshExpandedPath" -ArgumentList '-noprofile -c `"$ScriptBlock`"' -PassThru -Wait 
        `$stop = [datetime]::now
        `$procs += @{ StartTime = `$start; ExitTime = `$stop; CPU = 0 }
      }
      else {
         `$procs += { Start-Process -FilePath "$pwshExpandedPath" -ArgumentList '-noprofile -c `"$ScriptBlock`"' -PassThru -Wait }
      }
    }
